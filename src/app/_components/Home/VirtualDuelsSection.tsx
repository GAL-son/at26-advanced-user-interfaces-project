...
export default function VirtualDuelsSection({
  ...
  const { registerItem, handleKeyDown } = useKeyboardNavigation({
    ...
    orientation: "vertical", // Przywrócono pionowy zgodnie z fizycznym układem kart
    loop: false,
    onLeave: (direction) => {
  ...
  const renderDuelCard = (duel: VirtualDuel | null) => {
    ...
    return (
      <Link
        ...
        tabIndex={0}
        // UŻYCIE NOWEJ KLASY UTILITY: focus-brand
        className="group relative flex flex-col p-4 rounded-xl bg-[var(--color-brand-navy-dark)] border border-[var(--color-brand-navy-light)]/60 focus-brand hover:border-[var(--color-brand-yellow-hover)] hover:bg-[var(--color-brand-navy-light)]/30 overflow-hidden flex-1 h-full transition-colors duration-300"
      >
