...
export default function SocialsAndServers({
  ...
  return (
    <section 
      className="w-full bg-brand-navy-dark/20 rounded-xl p-5 border border-brand-navy-light/40 flex flex-col gap-5 animate-fadeIn focus:outline-none"
      aria-label={t("community.ariaLabel")}
    >
      {/* --- SEKCJA DISCORDA --- */}
      {discordLink && (
        <div className="flex flex-col gap-2">
          <h3 className="text-xs font-semibold uppercase tracking-wider text-brand-text-muted opacity-80">
          ...
          {(() => {
            ...
            return (
              <a
                ...
                tabIndex={0}
                /* Ujednolicenie rozmiaru tekstu przycisku pod natywny token --text-nav-button */
                className="flex items-center justify-center gap-2.5 w-full py-3 px-4 rounded-lg bg-[#5865F2] hover:bg-[#4752C4] text-white uppercase tracking-wide transition-colors duration-200 shadow-md focus-brand !text-nav-button font-bold"
              ...
              .map((item) => {
                ...
                return (
                  <a
                    ...
                    onKeyDown={(e) => handleKeyDown(e, itemIndex)}
                    tabIndex={0}
                    className="flex items-center justify-between p-3 rounded-lg bg-brand-navy-dark border border-brand-navy-light hover:border-brand-yellow-hover hover:bg-brand-navy-light/40 group transition-all duration-200 focus-brand"
                    aria-label={t("community.serverAriaLabel", { name: item.readableName })}
                  >
                    <div className="flex items-center gap-3 min-w-0">
                      <DnsIcon className="text-brand-text-muted group-hover:text-brand-yellow-hover transition-colors text-sm sm:text-base" />
                      {/* Podpięcie ujednoliconego fontu stałoszerokościowego z v4 */}
                      <span className="text-brand-text truncate !text-btn-mono font-medium sm:text-sm">
                        {item.readableName}
                      </span>
                    </div>
                    <span className="text-[11px] font-bold uppercase tracking-wider text-brand-yellow-hover opacity-0 group-hover:opacity-100 transition-opacity duration-200 hidden sm:inline">
