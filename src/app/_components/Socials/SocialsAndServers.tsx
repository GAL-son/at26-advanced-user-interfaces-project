...
interface SocialsAndServersProps {
  // Callbacki do nawigacji globalnej przekazywane z HomePage
...
export default function SocialsAndServers({
  ...
  if (discordLink) {
    interactiveItems.push({ id: "discord", type: "discord", url: discordLink });
  }
  ...
  const { registerItem, handleKeyDown } = useKeyboardNavigation({
    ...
    loop: false,
    // Kiedy użytkownik ucieka poza granice tego komponentu
    onLeave: (direction) => {
      if (direction === "prev" || direction === "next") {
        // Ruch w lewo/prawo (poprzednia/następna kolumna)
        if (onNavigateHorizontal) onNavigateHorizontal(direction);
      } else if (direction === "up" || direction === "down") {
        // Ruch w górę/dół (poprzedni/następny wiersz)
    ...
    >
      ...
      {discordLink && (
        <div className="flex flex-col gap-2">
          ...
          {(() => {
            // Znajdujemy indeks tego elementu w przefiltrowanej tablicy interaktywnej
            ...
            return (
              <a
                ...
                tabIndex={0}
                // focus-brand automatycznie obsłuży pierścień wokół przycisku
              ...
              .map((item) => {
                ...
                return (
                  <a
                    ...
                    onKeyDown={(e) => handleKeyDown(e, itemIndex)}
                    tabIndex={0}
                    // Implementacja uniwersalnej klasy focus-brand
                    ...
                    aria-label={`Przejdź do serwera ${item.readableName} (otwiera się w nowej karcie)`}
                  >
